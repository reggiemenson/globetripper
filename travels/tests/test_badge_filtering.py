...
class TestBadgeFiltering(SetBadgeData):
    ...
    def test_filtering_with_no_towns_returns_no_results(self):
        qualifying_badges = Badge.objects.get_qualifying_badges(towns=Town.objects.none())
    ...
    def testing_adding_town_adds_country_and_continent_badge(self):
        ...
        country_and_continent_badges = Badge.objects.get_qualifying_badges(towns=Town.objects.filter(id=self.london.id))
    ...
    def test_adding_countries_adds_matching_country_badges(self):
        ...
        towns = TownFactory.create_batch(100, name='one city', continent='one continent', capital='Not Primary', latitude=25)
        for i in range(0, 101, 10):
            with self.subTest(f"adding {i + 10}"):
                town_queryset = Town.objects.filter(id__in=[town.id for town in towns[i: i + 10]])
                ...
                num_of_country_badges = badge_result.filter(name__in=country_names).count()
    ...
    def test_adding_cities_adds_relevant_badges(self):
        ...
        for num, x in enumerate([5, 10, 50, 100, 150, 200, 500]):
            with self.subTest(f"adding {x}"):
                town_queryset = Town.objects.filter(id__in=[town.id for town in towns[:x]])
                badge_result = Badge.objects.get_qualifying_badges(towns=town_queryset)
    ...
    def test_special_criteria_badges(self):
        ...
        for i in range(6):
            ...
            with self.subTest(f"if visited {conditions[i]}..."):
                ...
                town_queryset = Town.objects.filter(id__in=[town.id for town in qualifying_towns])
                badge_result = Badge.objects.get_qualifying_badges(towns=town_queryset)
