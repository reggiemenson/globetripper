...
class TestLogin(AuthTestData):
    ...
    def test_successful_login_attempt(self):
        ...
        data = {"email": login_email, "password": self.password}
    ...
    def test_unknown_email_on_login_attempt(self):
        data = {"email": "random@email.com", "password": self.password}

        response = self.client.post(reverse(self.route_name), data=data)

        self.assertEqual(
            response.data.get("detail"), "Incorrect authentication credentials."
        )
    ...
    def test_incorrect_password_on_login_attempt(self):
        ...
        data = {"email": login_email, "password": "incorrectpassword"}
